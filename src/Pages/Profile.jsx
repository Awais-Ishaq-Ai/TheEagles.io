...
function Profile() {
  ...
  const socialMedia = [
    { id: 1, icon: <FaFacebook className="text-white" />, bgColor: 'bg-Background' },
    { id: 2, icon: <FaTwitter className="text-white" />, bgColor: 'bg-Background' },
    { id: 3, icon: <FaInstagram className="text-white" />, bgColor: 'bg-Background' },
    { id: 4, icon: <FaYoutube className="text-white" />, bgColor: 'bg-Background' },
    { id: 5, icon: <FaWhatsapp className="text-white" />, bgColor: 'bg-Background' }
  ];
      ...
      <div className="w-[95%] mx-auto">
        ...
        <div className="w-full h-[20vh] px-5 py-3 bg-Background rounded-lg my-4">
          ...
          <div className="h-[11vh] w-full my-2 rounded-lg flex justify-center items-center bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
            <div className="h-[70px] w-[99%] rounded-lg bg-Background">
              <button
          ...
          >
            <div className="bg-black w-[90%] max-w-[600px] p-6 rounded-lg relative">
              ...
              <div className="flex flex-col justify-center items-center py-6">
                <div>
                  <p className="text-white font-semibold text-xl">Add social links</p>
                  <p className="text-textColor2 font-medium text-sm text-center">Select social network</p>
                </div>

                <div className="flex space-x-6 my-8">
                  {socialMedia.map(item => (
                    <div key={item.id} className={`h-[30px] w-[30px] ${item.bgColor} rounded-full flex justify-center items-center`}>
                      {item.icon}
                    </div>
                  ))}
                </div>
                ...
                <button className="w-full py-2 bg-Background text-white rounded">Add Link</button>
              </div>
            </div>
          </div>
        )}

        <button className="text-white bg-gradient-to-r from-[#5b4fc6] to-[#170e61] w-[80%] py-2 rounded-lg mx-auto block" onClick={() => navigate('/')}>
          Save Changes
        </button>
      </div>
    </>
